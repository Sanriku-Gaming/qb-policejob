    ...
    if coords then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result)
            ...
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                ...
                TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
                exports['qb-inventory']:addTrunkItems(QBCore.Functions.GetPlate(veh), Config.CarItems)
                SetVehicleEngineOn(veh, true, true)
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                QBCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        ...
                        QBCore.Functions.Notify('Nothing unusual detected', "error")
                    end
                end, playerId)
            else
...
RegisterNetEvent('qb-police:fridge', function()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "police", Config.Food)
end)
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["fridge"]) do
            exports['qb-target']:AddBoxZone("PoliceFridge_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceFridge_" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                maxZ = v.z + 2,
