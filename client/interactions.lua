...
local function RearCuffAnimation()
...
local function GetCuffedAnimation(playerId)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "Cuff", 0.2)
    loadAnimDict("mp_arrest_paired")
    SetEntityCoords(ped, GetOffsetFromEntityInWorldCoords(cuffer, 0.0, 0.25, 0.0))
...
RegisterNetEvent('police:client:JailPlayer', function()
    QBCore.Functions.TriggerCallback('police:jail:nearbyPlayers', function(nearbyPlayers)
        if next(nearbyPlayers) ~= nil then
            ...
            if dialog then
                if not dialog.criminal or not dialog.jailTime then return end
                local jailed = {
                    crim = dialog.criminal,
                    time = dialog.jailTime,
                }
                TriggerServerEvent('police:server:JailPlayer', jailed)
            end
...
RegisterNetEvent('police:client:CuffPlayer', function(item)
    local cuffer = source
    if not IsPedRagdoll(PlayerPedId()) and not IsPedInAnyVehicle(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance <= 2.5 and not IsPedInAnyVehicle(GetPlayerPed(player)) then
            QBCore.Functions.TriggerCallback('police:server:getCuffStatus', function(cuffstat)
                ...
                if facing then
                    ...
                    TriggerServerEvent('police:server:CuffPlayer', "front", GetPlayerServerId(player), item, cuffer)
            ...
            if IsPedInAnyVehicle(GetPlayerPed(player)) then
                QBCore.Functions.Notify('You can\'t cuff someone sitting in a vehicle, jackass' , "error")
            else
                QBCore.Functions.Notify(Lang:t("error.none_nearby"), "error")
            end
        end
        ...
        if IsPedInAnyVehicle(PlayerPedId()) then
            QBCore.Functions.Notify('You can\'t cuff someone while in a vehicle, jackass' , "error")
        end
        Wait(2000)
    end
end)
...
RegisterNetEvent('police:client:AdminEscort', function(playerId)
    local ped = PlayerPedId()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if not isEscorted then
            isEscorted = true
...
RegisterNetEvent('police:client:GetCuffed', function(source, position, item)
    ...
    if not isHandcuffed then
        if position ~= 'front' and item == 'handcuffs' then
            GetCuffedAnimation()
        end
        if attempts < Config.BreakOutAttempts and Config.BreakOutCuffing then
            exports['ps-ui']:Circle(function(success)
                if not success then
                    ...
                    if Config.CuffItems[item].needkey then TriggerServerEvent('police:server:AddRemove', Config.CuffKeyItem, 1, "add", SecKey, cuffer) end
                    TriggerServerEvent('police:server:AddRemove', Config.CuffItems[item].itemname, 1, "remove", SecKey, cuffer)
                    TriggerServerEvent('police:server:cuffNotify', item, "cuffed", cuffer, true)
                else
                    ClearPedTasksImmediately(PlayerPedId())
                    attempts += 1
                    QBCore.Functions.Notify("You managed to break free", "success")
                    TriggerServerEvent('police:server:cuffNotify', item, "cuffed", cuffer, false)
                end
            end, 2, 10)
        else
            isHandcuffed = true
        ...
        if tostring(item) == Config.CuffKeyItem or tostring(item) == Config.CutTieItem then
            ...
            TriggerServerEvent('police:server:cuffNotify', item, "uncuffed", cuffer, true)
        end
    end
end)
...
CreateThread(function()
    while true do
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), animDict, animName, 3) and not IsEntityPlayingAnim(PlayerPedId(), "mp_arrest_paired", "crook_p2_back_right", 3)) and not (QBCore.Functions.GetPlayerData().metadata["isdead"] or QBCore.Functions.GetPlayerData().metadata["inlaststand"]) then
                loadAnimDict(animDict)
...
CreateThread(function()
    while true do
        Wait(Config.BreakOutTimeToReset * 60 * 1000)
        if attempts > 0 then
            attempts -= 1
        end
    end
