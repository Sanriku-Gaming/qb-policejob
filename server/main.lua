...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty) or Player.PlayerData.job.name == "admin" then
...
QBCore.Functions.CreateUseableItem('filled_evidence_bag', function(source, item)
    ...
    if Player and Player.PlayerData.job.type == "leo" then
        TriggerClientEvent('evidence:client:writeEvidenceNote', src, item)
    end
end)

-- Callbacks
QBCore.Functions.CreateCallback('police:jail:nearbyPlayers', function(source, cb)
...
QBCore.Functions.CreateCallback('police:server:isPlayerDeadorDying', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local dead = Player.PlayerData.metadata["isdead"]
    local laststand = Player.PlayerData.metadata["inlaststand"]
    if dead then cb(dead)
    elseif laststand then cb(laststand)
    else cb(false)
    end
end)
...
RegisterNetEvent('police:server:cuffNotify', function(item, type, cuffer, cuffed)
    local itemName = QBCore.Shared.Items[item].label
    if type == "cuffed" then
        if cuffed then
            TriggerClientEvent('QBCore:Notify', cuffer, itemName..' have been successfully applied!', 'success', 5000)
        else
            TriggerClientEvent('QBCore:Notify', cuffer, 'They broke free!', 'error', 10000)
        end
    else
        TriggerClientEvent('QBCore:Notify', cuffer, 'You have uncuffed them!', 'success', 5000)
    end
end)
...
RegisterNetEvent('police:server:KidnapPlayer', function(playerId)
    ...
    if not EscortPlayer.PlayerData.metadata["isdead"] or not EscortPlayer.PlayerData.metadata["inlaststand"] then
...
RegisterNetEvent('police:server:JailPlayer', function(jailed)
    local src = source
    local crim = tonumber(jailed.crim)
    local time = tonumber(jailed.time)
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(crim)
    --if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.name ~= "admin" then return end
...
RegisterNetEvent('evidence:server:DestroyCasing', function(casingId)
    TriggerClientEvent("evidence:client:RemoveCasing", -1, casingId)
    Casings[casingId] = nil
end)
...
RegisterNetEvent('police:server:AddRemove', function(itemname, amount, action, hash, cuffer)
    local source = source
    if cuffer ~= nil then source = cuffer end
    local Player = QBCore.Functions.GetPlayer(source)
